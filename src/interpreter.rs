...
pub enum Value {
    ...
    ChannelSender(std::sync::mpsc::Sender<Value>),
    ChannelReceiver(std::sync::Arc<std::sync::Mutex<std::sync::mpsc::Receiver<Value>>>),
...
impl Interpreter {
    pub fn new() -> Self {
        ...
        // For now, we'll handle print specially
    ...
    fn call_function(&mut self, call: FunctionCall) -> Result<Value, String> {
        ...
        if call.name == "channel" {
             ...
             return Ok(Value::List(vec![
                 ...
                 Value::ChannelReceiver(std::sync::Arc::new(std::sync::Mutex::new(rx)))
             ]));
        }
        ...
        if call.name == "send" {
            ...
            let sender = self.evaluate_expression(call.args[0].clone())?;
            let value = self.evaluate_expression(call.args[1].clone())?;
            
            if let Value::ChannelSender(tx) = sender {
                tx.send(value).map_err(|e| format!("Send error: {}", e))?;
                return Ok(Value::Nil);
            } else {
                return Err("First argument to send must be a sender".to_string());
            }
        }
        ...
        if call.name == "recv" {
            ...
            let receiver = self.evaluate_expression(call.args[0].clone())?;
            
            if let Value::ChannelReceiver(rx) = receiver {
                let rx = rx.lock().map_err(|e| format!("Lock error: {}", e))?;
                let value = rx.recv().map_err(|e| format!("Recv error: {}", e))?;
                return Ok(value);
            } else {
                return Err("First argument to recv must be a receiver".to_string());
            }
        }
...
mod tests {
    ...
    fn test_quantum() {
        let source = "
            ...
            return result
        ";
        // Result should be 0 or 1 with 50% probability
