...
impl Interpreter {
    ...
    fn execute_statement(&mut self, stmt: Statement) -> Result<Option<Value>, String> {
        ...
        match stmt {
            Statement::HtmlBlock(_) => Ok(None),
            Statement::WindowDecl(_) => Ok(None),
            Statement::MaterialDecl(_) => Ok(None),
    ...
    pub fn call_function(&mut self, call: FunctionCall) -> Result<Value, String> {
        ...
        match call.name.as_str() {
            "len" | "string_length" => {
                ...
                let arg = self.evaluate_expression(call.args[0].clone())?;
                match arg {
            ...
            "char_at" => {
                ...
                let str_val = self.evaluate_expression(call.args[0].clone())?;
                let idx_val = self.evaluate_expression(call.args[1].clone())?;
                ...
                match (str_val, idx_val) {
                    ...
                    _ => Err("char_at expects (String, Int)".to_string()),
                }
            }
            "sleep" => {
                if call.args.len() != 1 {
    ...
    pub fn execute_value_call(&mut self, func_value: Value, args: Vec<Value>, explicit_type_args: Vec<Type>, name_hint: Option<String>) -> Result<Value, String> {
        match func_value {
    ...
    fn apply_binary_op(&self, left: Value, op: Operator, right: Value) -> Result<Value, String> {
        ...
        match op {
            Operator::Add => match (&left, &right) {
                ...
                _ => Err(format!("Type mismatch in addition: {:?} + {:?}", left, right)),
            },
            ...
            Operator::And | Operator::Or => {
                // These are handled by short-circuiting in evaluate_expression
